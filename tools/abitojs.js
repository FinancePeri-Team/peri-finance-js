 ...
 *    (based on the mainnet release of periFinance - from the dependency)
...
const contracts = {
  ...
  Pynth: {
    target: 'ProxyERC20pUSD',
  },
  PeriFinance: {
    target: 'ProxyERC20',
  },
  PeriFinanceEscrow: true,
  PeriFinanceState: true,
  RewardEscrow: true,
...
const writeAddressFile = () => {
  const addressDefinitions = Object.values(SUPPORTED_NETWORKS)
    .map(network => {
      const targets = peri.getTarget({ network });
...
const writePynthsFile = () => {
  const pynthDefinitions = Object.values(SUPPORTED_NETWORKS)
    .map(network => {
      const pynths = peri.getPynths({ network });

      return `
        const ${network.toUpperCase()}_PYNTHS = ${util.inspect(pynths, {
        showHidden: false,
        depth: null,
      })}
        `;
  ...
  const exportFooter = `
    export default {
    ${Object.entries(SUPPORTED_NETWORKS)
      .map(([networkId, network]) => `${networkId}: ${network.toUpperCase()}_PYNTHS`)
      .join(', ')}
    };`;

  fs.writeFile(
    path.join(__dirname, '..', 'lib', 'pynths.js'),
    pynthDefinitions + exportFooter,
    err => {
      if (err) {
        console.log(err);
      } else {
        console.log(`lib/pynths.js successfully generated.`);
      }
    }
  );
};
...
const generate = () => {
  ...
  Object.values(SUPPORTED_NETWORKS).map(network => {
    ...
    Object.keys(allContracts).forEach(contractName => {
      ...
      // get the abis from the network's deploy from periFinance
...
const writeAllABIFiles = network => {
  ...
  const sources = peri.getSource({ network });
