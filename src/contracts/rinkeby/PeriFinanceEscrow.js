...
import abi from '../../../lib/abis/rinkeby/PeriFinanceEscrow';
...
function PeriFinanceEscrow(contractSettings) {
  ...
  /**
   * This may only be called by the owner during the contract's setup period., Withdraws a quantity of PERI back to the periFinance contract.<br>
   * Transaction (consumes gas, requires signer)
   * @param quantity {BigNumber}
   * @param txParams {TxParams}
  
   **/
  this.withdrawPeriFinance = async (quantity, txParams) => {
    txParams = txParams || {};
    return await this.contract.withdrawPeriFinance(quantity, txParams);
  };
