...
import abi from '../../../lib/abis/rinkeby/Pynth';
...
function sRUB(contractSettings) {
   ...
   * @returns String<EthAddress>
   **/
  this.periFinancece = async () => {
    return await this.contract.periFinancece();
  };
  ...
  /**
   * Transaction (consumes gas, requires signer)
   * @param _periFinancece {String<EthAddress>}
   * @param txParams {TxParams}
  
   **/
  this.setPeriFinance = async (_periFinancece, txParams) => {
    txParams = txParams || {};
    return await this.contract.setPeriFinance(_periFinancece, txParams);
  };
}
