...
import abi from '../../../lib/abis/polygon/SystemSettings';
...
function SystemSettings(contractSettings) {
  ...
  /**
   * Transaction (consumes gas, requires signer)
   * @param pynthKeys {bytes32[]}
   * @param exchangeFeeRates {uint256[]}
   * @param txParams {TxParams}
  
   **/
  this.setExchangeFeeRateForPynths = async (pynthKeys, exchangeFeeRates, txParams) => {
    txParams = txParams || {};
    return await this.contract.setExchangeFeeRateForPynths(pynthKeys, exchangeFeeRates, txParams);
  };
